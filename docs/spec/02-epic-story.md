...
| 모바일 배포 채널 | **Google Play 후보** — 등록비 $25는 **최초 1회뿐**(연회비 없음)이라 「연 0원」과 충돌하지 않는다. ~~Apple App Store~~는 iOS 범위 밖으로 소멸 | ⏸ **최종단계에 검토**(2026-08-12 결정). ⚠️ 아래 「Play 계정 — 최종단계 함정」 |
...
- [ ] `CE5-S1` **Desktop** — `electron-updater`(피드 URL·서명·롤백). **채널 = GitHub Releases 확정(2026-08-12)**. 착수 전 아래 3건이 선행돼야 실제로 동작한다
  ...
  - 🔴 **macOS 코드 서명 + 공증** 〔G-SIGN〕 — 문서 원문: *"macOS 앱은 자동업데이트가 동작하려면 반드시 서명되어야 한다"*. Windows는 미서명으로도 업데이트 자체는 동작(SmartScreen 경고만)
    ...
    - Windows는 미서명으로도 자동업데이트가 동작한다. 다만 `win.verifyUpdateCodeSignature`가 **기본 `true`**라 미서명 빌드에서는 **`false`로 꺼야 한다**(실측 확인 — `winOptions.d.ts:74`)
...
| **G-DIST** 배포 채널 | CE2-S4 | 배포 채널 확정 | ✅ **데스크톱 통과** — GitHub Releases + `electron-updater`(2026-08-12). 모바일은 ⏸ **최종단계 검토**로 이월(Play $25는 1회성이라 방침과 충돌 없음). Apple App Store는 iOS 범위 밖으로 소멸 |
...
**2026-08-12 결정: Google Play 사용 여부는 최종단계에 검토한다.** 등록비 **$25는 최초 1회뿐**(연회비 없음)이라 「연 0원」 방침과 충돌하지 않는다. 앱이 무료이고 인앱결제도 없으므로 Google 수수료(15~30%)도 해당 없다.
...
즉 최종단계에 가서 개인 계정으로 정하면, **그 시점부터** 테스터 12명을 모아 14일을 기다려야 한다. 출시 직전에 붙는 2주다.
...
- **Wave 0 〔결정〕 완료(2026-08-12)** — ~~G-ID~~ ✅ · ~~G-DOM~~ ✅ · ~~G-IOS~~ ⬛ 폐기(iOS 범위 밖) · ~~G-SIGN~~ ✅ 연 0원 = 미서명 배포 · ~~G-DIST~~ ✅ 데스크톱 확정(모바일 채널은 ⏸ 최종단계 이월) → **착수를 막는 결정은 더 없다**
