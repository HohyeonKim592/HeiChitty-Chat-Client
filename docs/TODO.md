...
- [x] **① `win.verifyUpdateCodeSignature: false`** — 2026-08-14 반영. 기본값이 `true`라 미서명 빌드에서는 업데이트 설치가 거부된다(실측: `winOptions.d.ts:74`)
- [x] **② `publish`에 `owner`/`repo` 명시** — 2026-08-14 반영(`HohyeonKim592`/`HeiChitty-Chat-Client`). `package.json`의 `repository` URL 추론에 기대던 것을 끊었다
- [x] **③ `electron/src/index.ts`의 `.catch()` 삼킴 재검토** — 2026-08-14 완료. **결론: 유지 + 호출 범위 축소**
  - `.catch()` **유지** — public 전환으로 피드 접근은 열렸으나 **릴리스가 아직 0건**이고 오프라인 기동도 있다. 실패가 unhandled rejection 이 되면 모달로 앱이 멈추는 구조(2026-08-10 사고)는 그대로다
  - **`process.platform === 'win32'`일 때만 호출** — macOS 는 미서명이라 Squirrel.Mac 이 성립하지 않는다. 매번 실패가 확정된 요청을 보내지 않는다
- [–] ~~mac 타깃에 `zip` 추가~~ — macOS 자동업데이트를 포기했으므로 불필요해짐

> 선행 3건은 끝났다. 남은 `CE5-S1` 본체는 **실제 GitHub Release 를 올려 업데이트 수신·적용을 확인**하는 일이며, Windows 실기가 전제다(아래 검증 후보).

### 모바일 채널 — ⏸ **실기기 테스트 이후 결정** (2026-08-14)

> **Play 업로드 여부는 Android 실기기 테스트를 마친 뒤 정한다**(2026-08-14 결정). 뷰어가 실기기에서 제대로 도는지 확인되기 전에는 배포 채널을 정할 근거가 없다.
> 등록비 **$25는 최초 1회뿐**(연회비 없음)이라 「연 0원」 방침과 충돌하지 않는다. 무료 앱이라 Google 수수료도 무관.
> 상세·완화책은 `spec/02-epic-story.md` 「Play 계정 — 최종단계 함정」.

- [ ] **선행: Android 실기기 테스트** — 이것이 결정의 입력이다(아래 검증 후보의 「Android 실기」·「Android 빌드 산출물 실행」)
- [ ] **⚠️ 다만 Play 를 쓰기로 하면 계정 종류가 곧바로 일정에 붙는다** — **개인 계정은 「테스터 12명 × 연속 14일」 의무**이고, **계정을 나중에 만들어도 피할 수 없다**(2023-11-13 이후 생성분 전부 적용). 조직 계정만 면제.
  즉 실기기 테스트 후 개인 계정으로 정하면 **그 시점부터 2주**다. 계정 종류만이라도 먼저 가늠해 두면 대기시간이 다른 작업과 겹쳐 사라진다
...
- [x] **`${os}` 매크로 실기 검증** — 2026-08-14 mac 빌드로 **확인 완료**. `appOutDir=…/release/mac/mac-universal`, 산출물이 `release/mac/`에 생성됨. win 은 미확인이나 같은 매크로라 동일하게 동작할 것으로 본다
...
- **Wave 0 결정 완료** — G-ID·G-DOM·G-SIGN·G-DIST(데스크톱) 확정, G-IOS 폐기, 모바일 채널은 Android 실기기 테스트 이후로 이월. **착수를 막는 결정은 더 없다**
